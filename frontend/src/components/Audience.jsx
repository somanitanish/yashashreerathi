  ...
  return (
    ...
    >
      <div className="container-x">
        ...
        <p className="mt-8 max-w-2xl text-base" style={{ color: "#B9B9C0", lineHeight: 1.7 }}>
        ...
        <div className="grid lg:grid-cols-2 gap-8 mt-16">
          ...
          <div className="flex flex-col gap-5">
            {personas.map(({ Icon, title, body }) => (
              <div
                ...
                className="card-hover rounded-2xl p-6 flex gap-5 items-start"
                style={{ background: "#1A1A1D", border: "1px solid #26262A" }}
              >
                <div
                  className="flex items-center justify-center rounded-xl flex-shrink-0"
                  style={{ width: 56, height: 56, background: "var(--yellow)" }}
                >
                  <Icon size={28} color="#0E0E10" strokeWidth={2.2} />
                </div>
                <div>
                  <h3 className="text-lg font-bold">{title}</h3>
                  <p className="mt-2 text-sm" style={{ color: "#A8A8B0", lineHeight: 1.6 }}>
                    {body}
                  </p>
                </div>
              </div>
          ...
          <div className="flex flex-col gap-6">
            ...
            >
              <h3 className="text-lg font-bold">Platform Distribution</h3>
              <div className="mt-6 space-y-7">
                {platforms.map((p) => (
                  <div key={p.name}>
                    <div className="flex justify-between items-baseline">
                      <span className="font-medium">{p.name}</span>
                      <span className="font-display" style={{ color: "var(--pink)", fontSize: "1.4rem" }}>
                        {p.value}
                      </span>
                    </div>
            ...
            <div className="grid grid-cols-4 gap-3">
              {countries.map((c) => (
                <div
                  ...
                  style={{ background: "#1A1A1D", border: "1px solid #26262A" }}
                >
                  <p className="font-display" style={{ fontSize: "1.6rem", color: "#3A3A40" }}>
                    {c.code}
                  </p>
                  <p className="mt-1 text-xs" style={{ color: "#A8A8B0" }}>
                    {c.label}
                  </p>
                </div>
              ))}
